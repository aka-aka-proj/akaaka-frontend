...
- **Push 前 local validation gate：** 版本控制的 `.githooks/pre-push` 會自動執行固定的 `npm run lint`、`npm test` 與 `npm run build`；若偵測到 UI、路由、responsive、browser interaction、Playwright fixture、`src/`、`tests/` 或 Playwright 設定變更，hook 會再執行 Chromium Playwright 與 `npm run test:manifest`。任何 hook 失敗不得 push，也不得使用 `--no-verify` 繞過；若變更影響 Lighthouse gate，agent 仍須額外執行 `rtk npm run test:lighthouse`。local hook 通過只代表 push 前預檢，不能取代 GitHub required checks、review、deployment 或 post-validation。新 clone／worktree 執行 `npm ci` 時由 `prepare` 自動啟用此 hook。
