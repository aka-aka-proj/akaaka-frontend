...
export function EventSchedulingPollPage() {
  ...
  const confirmVoteInvalidation = () => {
    if (!Object.values(counts).some((count) => count > 0)) return true
  ...
  const addDate = async () => {
    if (!poll || !candidateDate || !confirmVoteInvalidation()) return
  ...
  const addLocation = async () => {
    if (!poll || !candidateLocation.trim() || !confirmVoteInvalidation()) return
  ...
  const addVoter = (profile: ProfileResult) => poll && confirmVoteInvalidation() && run(() => supabase.from('event_scheduling_poll_voters').insert({ poll_id: poll.id, profile_id: profile.id }), t('schedulingPoll.voterAdded'))
